...
class comm3_stat extends fs_model
{
   ...
   {
      $data = $this->db->select("SELECT * FROM comm3_stats WHERE email = ".$this->var2str($email).";");
      if($data)
      {
   ...
   {
      $data = $this->db->select("SELECT * FROM comm3_stats WHERE rid = ".$this->var2str($rid).";");
      if($data)
      {
         return new comm3_stat($data[0]);
      }
   ...
   public function exists()
   {
      if( is_null($this->email) )
      {
         ...
         return $this->db->select("SELECT * FROM comm3_stats WHERE email = ".$this->var2str($this->email).";");
   }
   ...
   public function save()
   {
      ...
      {
         $sql = "INSERT INTO comm3_stats (email,perfil,codpais,nick,last_login,last_login_time,last_ip,last_browser,rid)
   ...
   {
      return $this->db->exec("DELETE FROM comm3_stats WHERE email = ".$this->var2str($this->email).";");
   }
   ...
   public function all()
   {
      ...
      $data = $this->db->select("SELECT * FROM comm3_stats ORDER BY last_login DESC, last_login_time DESC;");
      if($data)
      {
         foreach($data as $d)
